 ...
 */
public class mSignInPage extends SignInPageBase {
    @Override
    ...
    public void EnterEmailAddressMobile(String email) throws Exception {
        StepLogger.validate(Check.ObjectPresent, "emailAddressMobile_AAF");
        Find.Object("emailAddressMobile_AAF").clear();
        Find.Object("emailAddressMobile_AAF").input(email);
        if(Find.Object("submitButton_AAF").isPresent()){
            Report.pass("User Clicks on Submit Button", BrowserAction.screenshot());
            Find.Object("submitButton_AAF").click();
        }else{
            Report.fail("Submit Button is NOT available");
        }
    }
